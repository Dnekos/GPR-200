...
/*
	...
	Modified because: Needed main to write the graphics to file
*/
...
int main() {
	...
	// World
	Hittable_List world; // create list of hittables
	world.add(make_shared<Sphere>(point3(0, 0, -1), 0.5f)); // create sphere
	world.add(make_shared<Sphere>(point3(0, -100.5, -1), 100.0f)); // create 'ground' Sphere
	...
	for (int j = image_height - 1; j >= 0; --j) // iterates vertically
	{
		std::cerr << "\rScanlines remaining: " << j << ' ' << std::flush;
		for (int i = 0; i < image_width; ++i) // iterates horizontally
		{
			color pixel_color(0, 0, 0); // create color variable
